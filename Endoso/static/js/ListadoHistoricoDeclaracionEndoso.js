...
function inicio(){	// Función que inicia las variables y los eventos de los controles del formulario historico declaracion de endoso.​
	activarMenu();
...
function buscarConstancia(){ // Función que busca la lista de constancias en base en base al id de la constancia.
	...
	{
		Dajaxice.Endoso.obtener_constancia_idconstancia(cargarModalConstancias,{'idConstancia':$("#txtBuscarConstancia").val()});	
		return false;
	}
};
...
function tabla_buscar_constancia(){ // Función que permite interactuar con la ventana modal de buscar constancias.
	...
	$('.tbl_buscar_constancia tbody').on('click', 'tr', function(event) { //evento click que selecciona el row y marca el check de la tabla		
		...
		$(".close").click();
		buscarDeclaracionesEndososConstancia();
	});
}

function buscarDeclaracionesEndososConstancia() // Función que busca la lista de constancias en base en base al id de la constancia.
{
	Dajaxice.Endoso.obtener_declaraciones_endoso_idconstancia(cargarDeclaracionesEndoso,{'idConstancia':$("#varIdConstancia").val()});	
	return false;
}

function cargarDeclaracionesEndoso(data) // Función que nos permite cargar el listado de declaraciones de endoso.
{
	if(data.declaracionendoso.length > 0)
	{	
		var items = [];
		...
		{	
			...
			linkVerDeclaracion = "<a href='#' title='Imprimir Declaracion' onclick='imprimirDeclaracionEndoso($(this));'><i class='icon-th-list'></i></a>";
			linkVerEndoso = "<a href='#' title='Imprimir Endoso' onclick='imprimirEndoso($(this));'><i class='icon-list-alt'></i></a>";
			
			items.push({Rfc:"Rfc: " + declaracionendoso[rowArreglo][5], Nombre:"Nombre: " + declaracionendoso[rowArreglo][4], IdEndoso:declaracionendoso[rowArreglo][0], FolioConstancia:"Constancia: " + declaracionendoso[rowArreglo][2], NumeroSolicitud:"Solicitud: " + declaracionendoso[rowArreglo][3], Mes:meses[mes-1].toUpperCase() + ' DEL ' + anoActual, IdConstancia:declaracionendoso[rowArreglo][1], accionesVerDeclaracion:linkVerDeclaracion, accionesVerEndoso:linkVerEndoso});
		}
	...
	{
		alertify.success('No se encontraron declaraciones de endoso');
	}
}

function imprimirDeclaracionEndoso(id_declaracion) // Función que nos permite imprimir la declaración de endoso.
{
	declaracionSeleccionada = id_declaracion.closest("li");
	window.open('/DeclaracionEndosoImpresion/' + declaracionSeleccionada.children()[0].innerText);
}

function imprimirEndoso(id_declaracion) // Función que nos permite imprimir la información del endoso.
{
	declaracionSeleccionada = id_declaracion.closest("li");
	window.open('/EndosoDeclaracionImpresion/' + declaracionSeleccionada.children()[0].innerText);
}